...
binTree* buildPostfix(const char* expression) {
    ...
    for (int i = 0; expression[i] != '\0'; ++i) {
        ...
        if (isspace(token)) continue;     // a built in function to check if the current character is a space.

        if (isalnum(token)) {             // a built in function that checks if the current character is alphanumeric
            // Operand
            binTree* operand = createNode(token);   //pushes the current operand to the top of the stack.
            push(&stack, operand);
...
binTree* buildInfix(const char* expression) {
    char postfix[100] = "";         // Stores the postfix expression
